...
<div class="col-lg-6 col-md-6">
	<div class="product_d_right">
			...
			<div class="price_box">
                ...
                <button style="font-size: 13px;" value="{{$size->size}}"  type="button" class="btn btn-light btn-sm size px-3 py-2 ml-0">{{$size->size}} </button> 
				@endforeach
			</div> 
			 @if($product->type == 'Without Design')
			<div class="product_variant quantit"> 
			 <form method="POST" action="{{route('base.shoe.size',$product->id)}}">
					@csrf
					<input type="hidden" name="size" id="size" value="">  
					{{-- <button id="button_cart_432095_buy_now" class="" type="submit" name="dispatch[checkout.add]">BUY NOW</button>  --}}
				   <button name="submit" id="cart_submit" class="ty-btn__secondary ty-btn__add-to-cart ty-btn" type="submit">Select & Continue</button>  
			</form> 
			</div>
			@endif
